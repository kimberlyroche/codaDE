...
my $baseline = "self";
my $model_dir = "self_nopartial_wFC_ABSOLUTE";
...
while($i <= $end) {
  ...
  print $fh 'srun Rscript validate.R --dataset='.$datasets[$i].' --baseline='.$baseline.' --threshold='.$threshold[$i].' --model_folder='.$model_dir."\n\n";
