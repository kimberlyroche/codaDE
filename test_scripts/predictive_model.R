...
output_file <- file.path("output", "results_singlecellRNAseq_PREV.tsv")
...
p1 <- ggplot(data) +
       geom_point(aes(x = log(prop_de), y = fpr))
p2 <- ggplot(data) +
       geom_point(aes(x = log(prop_de), y = fnr))
p3 <- ggplot(data) +
       geom_point(aes(x = log(sfcorr), y = fpr))
p4 <- ggplot(data) +
       geom_point(aes(x = log(sfcorr), y = fnr))
p <- grid.arrange(p1, p2, p3, p4, nrow = 2)
ggsave("predictor_log_correlation.png", p)

# (4) Fit and evaluate a Beta regression model.
# Note: We need to add a tiny value because betareg only allows outcomes (0,1).

addend <- 0.00001
data$fpr <- data$fpr + addend
data$fnr <- data$fnr + addend

fit_fpr <- betareg(fpr ~ prop_de + sfcorr, data = data, link = "log")
predicted_fpr <- predict(fit_fpr, data)

fit_fnr <- betareg(fnr ~ prop_de + sfcorr, data = data, link = "log")
predicted_fnr <- predict(fit_fnr, data)

plot_df <- data.frame(predicted_values = predicted_fpr, true_values = data$fpr, which = "fpr")
plot_df <- rbind(plot_df,
                 data.frame(predicted_values = predicted_fnr, true_values = data$fnr, which = "fnr"))
plot_df$which <- as.factor(plot_df$which)

p <- ggplot(plot_df) +
     geom_point(aes(x = true_values, y = predicted_values)) +
     facet_grid(cols = vars(which))
ggsave("betareg_model.png", p)
...
# x should be N x D
# y should be N x 1
x <- cbind(data$prop_de, data$sfcorr)
y <- data$fpr
