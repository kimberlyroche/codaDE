...
for(use_result_type in c("TPR", "FPR")) {
  ...
  # Overall
  cat(paste0("Overall R^2 (",use_result_type,"): ",
             round(cor(fit_obj$test_response, prediction)^2, 3), "\n"))

  for(p in sort(unique(test_data$p))) {
    # Per-P
    idx <- which(test_data$P == p)
    prediction <- predict(fit_obj$result, newdata = test_data[idx,]) # predict.all = TRUE

    cat(paste0("P=",p," R^2 (",use_result_type,"): ",
               round(cor(fit_obj$test_response[idx], prediction)^2, 3), "\n"))
  }
